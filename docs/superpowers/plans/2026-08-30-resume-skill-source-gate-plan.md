...
定制版生成后逐句核对：来源事实编号；动作和对象；贡献动词；数字和结果；待确认项去向。任一项无法通过时，从正文删除或移入追问、风险清单，不交付为最终简历句子。
```
